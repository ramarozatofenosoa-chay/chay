...
function ClockCard({ label, tz, flag }) {
  ...
  return (
    <div className="rounded-[1.5rem] border border-border bg-card p-5 text-center">
      ...
      <div className="font-display font-extrabold text-2xl md:text-3xl mt-1 tabular-nums">
        {time}
      </div>
    </div>
  );
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const events = (Array.isArray(anns) ? anns : []).filter(
          (a) => a.type === "event"
        );
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 md:px-8 py-8 md:py-12">
      ...
      <section className="mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <ClockCard label="Seattle" tz="America/Los_Angeles" flag="🇺🇸" />
      </section>
